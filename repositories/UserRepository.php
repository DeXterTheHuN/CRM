...
class UserRepository extends Repository
{
    ...
    {
        $stmt = $this->pdo->query("SELECT * FROM users ORDER BY approved ASC, role DESC, name ASC");
    ...
    {
        $stmt = $this->pdo->query("SELECT COUNT(*) AS cnt FROM users WHERE approved = 0");
    ...
    /**
     * Unapprove a user (set approved = 0).
     *
     * @param int $userId
     * @return void
     */
    ...
    {
        $stmt = $this->pdo->prepare("UPDATE users SET approved = 0 WHERE id = ?");
        $stmt->execute([$userId]);
    }
    ...
    {
        $stmt = $this->pdo->query("SELECT id, name FROM users WHERE approved = 1");
        return $stmt ? $stmt->fetchAll(PDO::FETCH_ASSOC) : [];
    }
     ...
     */
    public function createUser(string $username, string $passwordHash, string $name, ?string $email, string $role = 'agent', int $approved = 0): int
    {
        $stmt = $this->pdo->prepare(
            "INSERT INTO users (username, password, name, email, role, approved) VALUES (?, ?, ?, ?, ?, ?)"
        );
        $stmt->execute([$username, $passwordHash, $name, $email, $role, $approved]);
