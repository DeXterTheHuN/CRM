...
{
    /**
     * Get count of unread chat messages for a user.
     *
     * @param int $userId
     * @return int
     */
    public function getUnreadChatCount(int $userId): int
    {
        // Count unread chat messages by reusing ChatRepository logic but without instantiating ChatRepository directly
        // This method replicates logic from notifications_api.php for efficiency.
        $sql = "
            SELECT COUNT(*) AS unread_count
            FROM chat_messages cm
            LEFT JOIN chat_read_status crs ON crs.user_id = :user_id AND cm.id <= crs.last_read_message_id
            WHERE cm.user_id != :user_id2 AND crs.last_read_message_id IS NULL
        ";
    ...
    /**
     * Get the number of pending client approvals.
     * Only for admin.
     *
     * @return int
     */
    ...
    {
        $stmt = $this->pdo->query("SELECT COUNT(*) AS pending_count FROM clients WHERE approved = 0");
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function getNewClientsTotal(int $userId, bool $isAdmin): int
    {
        ...
        // Két külön paraméter: az egyik a client_views.user_id-hez, a másik az ügynök szűréséhez.
    ...
    public function markCountyClientsViewed(int $countyId, int $userId, bool $isAdmin): int
    {
        ...
        ";
        if (!$isAdmin) {
            $sql .= " AND (agent_id = :user_id OR agent_id IS NULL)";
        }
        $stmt = $this->pdo->prepare($sql);
    ...
    /**
     * Get the latest chat message from other users after a given time.
     *
     * @param int    $userId
     * @param string $afterTimestamp
     * @return array|null
     */
    public function getLatestChatMessage(int $userId, string $afterTimestamp): ?array
    {
        $sql = "
            SELECT cm.*, 
                   (SELECT last_read_message_id FROM chat_read_status WHERE user_id = :user_id LIMIT 1) AS last_read_id
            FROM chat_messages cm
            WHERE cm.user_id != :user_id2
        ...
        if (!$msg['last_read_id'] || $msg['id'] > $msg['last_read_id']) {
            return [
                'id' => $msg['id'],
                'user_name' => $msg['user_name'],
                'message' => $msg['message'],
                'created_at' => $msg['created_at'],
                'last_read_id' => $msg['last_read_id']
            ];
        }
        return null;
    }
