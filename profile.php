...
// Felhasználó adatainak lekérdezése repository segítségével
$user = $userRepo->getUserById((int) $_SESSION['user_id']);

if (!$user) {
    ErrorHandler::logAppError('User not found in profile', [
        'user_id' => $_SESSION['user_id']
    ]);
    $_SESSION['error'] = 'Felhasználó nem található';
    redirect('logout.php'); // Kijelentkeztetjük, mert nincs ilyen user
}
    ...
    if (!$name) {
        ...
        if ($new_password) {
            if (!$current_password) {
                ...
                // Jelszó frissítése repository segítségével
                $hashed_password = password_hash($new_password, PASSWORD_DEFAULT);
                $userRepo->updateUser((int) $_SESSION['user_id'], $name, $email, $hashed_password);
                $_SESSION['name'] = $name;
            ...
            // Csak név és email frissítése repository segítségével
            $userRepo->updateUser((int) $_SESSION['user_id'], $name, $email, null);
            $_SESSION['name'] = $name;
                        ...
                        <div class="mb-4">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control"
                                    value="<?php echo $user['role'] === 'admin' ? 'Adminisztrátor' : 'Ügyintéző'; ?>"
                                    disabled>
