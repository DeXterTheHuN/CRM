...
function cache_get(string $key, int $ttl = CACHE_TTL_DEFAULT)
{
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        ...
        // Error logging
        ErrorHandler::logAppError('Cache JSON decode error', [
            'key' => $key,
...
function cache_set(string $key, $data): bool
{
    ...
    if (!is_dir($cacheDir)) {
        if (!@mkdir($cacheDir, CACHE_DIR_PERMISSIONS, true)) {
            ErrorHandler::logAppError('Failed to create cache directory', [
                'dir' => $cacheDir,
                'permissions' => decoct(CACHE_DIR_PERMISSIONS)
            ]);
            return false;
        }
    }
    ...
    if ($encoded === false) {
        ErrorHandler::logAppError('Cache JSON encode error', [
            'key' => $key,
            'error' => json_last_error_msg()
        ]);
        return false;
